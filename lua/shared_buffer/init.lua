...
-- FIXED: Changed parameter name from 'type' to 'buffer_type' to avoid shadowing Lua's type() function
function M.setup(buffer_type)
    if not buffer_type or type(buffer_type) ~= "string" or buffer_type == "" then
        vim.notify(M.error_messages.INVALID_TYPE, vim.log.levels.ERROR)
        return nil
    end

    local state_file = vim.fn.stdpath("data") .. "/shared_" .. buffer_type .. "_state.json"
...
-- FIXED: Changed parameter name from 'type' to 'buffer_type' to avoid shadowing Lua's type() function
function M.setupWBuff(buffer_type)
    if not buffer_type or type(buffer_type) ~= "string" or buffer_type == "" then
        vim.notify(M.error_messages.INVALID_TYPE, vim.log.levels.ERROR)
        return nil
    end
    ...
    -- File paths
    local content_file = vim.fn.stdpath("data") .. "/shared_" .. buffer_type .. "_buf_content.txt"
    local state_file = vim.fn.stdpath("data") .. "/shared_" .. buffer_type .. "_state.json"
